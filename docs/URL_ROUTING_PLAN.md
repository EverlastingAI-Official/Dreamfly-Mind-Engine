...
状态：2026-09-25 已实施。下文保留设计和验收范围，实际结构与注意事项见文末。
...
1. **统一导航入口。** 增加 `src/services/navigation.mjs`，集中声明页面路径、查询参数读写和分享地址生成。`locale.js` 不再负责路由；删除组件直接改写浏览历史的逻辑。
...
- 后续整理已移除 GitHub 同步页面及导航；旧 `/sync/github` 和 `view=github` 入口迁移到 `/skills/mine`。成功同步后的 GitHub 入口改为显示在对应 Skill 详情中。

- `src/index.vue` 仅负责根入口跳转。uni-app 会把第一个注册页别名设为 `/`，因此不能直接把探索页放在第一项，否则导航会再次变成 `/?search=...`。
- `createRoutePage.js` 将原生页面生命周期和 URL 查询传入 `PlatformWorkspace.vue`；后者组合共用布局与业务面板。Skill 编辑、模型连接、会话逻辑分别放在独立 composable，`useWorkspace.js` 处理会话恢复、页面加载和共用操作。
- `navigation.mjs` 沿用框架自带 router。由于当前 uni-app 会重复编码查询参数，导航入口使用一次性路由重定向恢复标准查询，同时保留框架的页面栈状态；页面从 router 读取已解码参数，避免 onLoad 再次解码。中文、`+`、`&`、`%` 和登录 returnTo 均经浏览器验证。
- 仅在应用启动迁移旧地址时调用 `history.replaceState`；页面切换统一通过导航模块。探索列表的筛选、排序、页码来自 URL；编辑中的表单、导入内容和未提交消息不放入地址栏。
- 托管入口为 `scripts/serve-h5.mjs`，配置与部署边界见 `LOCAL_DEVELOPMENT.md`。H5 与服务端构建、路由定向测试已通过；真实收费模型调用和远程部署不在本次验证范围内。
