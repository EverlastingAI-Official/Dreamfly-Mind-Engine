...
查询与反馈集中在 `server/src/discovery.ts`，公开内容投影复用于详情和导出。新增 `004_skill_reactions.sql`，本地已应用。`ExploreSkills.vue` 保持独立浏览状态，共享样式从页面移至应用层，修复 uni-app 自动作用域导致子组件卡片/按钮样式缺失的问题。
