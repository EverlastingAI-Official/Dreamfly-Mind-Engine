...
import { body, check, text } from '../errors.js';
...
const email = (v: unknown) => {
  ...
  check(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e), 422, 'INVALID_EMAIL', '邮箱格式无效');
  return e;
};
...
async function consume(db: DB, b: Credentials, purpose: string) {
...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.post('/auth/email-codes', { ...pub, schema: { body: bodies.emailCode } }, async (r) => {
    const b = body<Credentials>(r),
      e = email(b.email);
    check(
      ['register', 'reset_password'].includes(b.purpose),
      422,
      'INVALID_PURPOSE',
      '验证码用途无效',
    );
  ...
  app.post('/auth/register', { ...pub, schema: { body: bodies.register } }, async (r) => {
    const b = body<Credentials>(r),
      e = email(b.email),
    ...
    check(success, 422, 'INVALID_CODE', '验证码无效、已使用或账号已存在');
    return { message: '注册成功，请登录' };
  });
  app.post('/auth/login', { ...pub, schema: { body: bodies.login } }, async (r, reply) => {
    const b = body<Credentials>(r),
      e = email(b.email);
    check(
  ...
  app.post('/auth/logout', (r, p) => logout(r, p));
  app.post('/auth/logout-all', (r, p) => logout(r, p, true));
  app.post(
    '/auth/reset-password',
    ...
    async (r) => {
      ...
      check(success, 422, 'INVALID_CODE', '验证码无效或已过期');
      return { message: '密码已重置，请重新登录' };
    },
  );
  app.post('/auth/change-password', { schema: { body: bodies.changePassword } }, async (r, p) => {
    const b = body<Credentials>(r),
      hash = await argon2.hash(password(b.password));
