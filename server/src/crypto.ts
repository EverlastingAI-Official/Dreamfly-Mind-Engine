...
export const codeDigest = (value: string) =>
  createHmac('sha256', secret('EMAIL_CODE_SECRET', true)).update(value).digest('hex');
...
function key(version: string) {
  const raw =
    version === config.keyVersion
      ...
      : secret(`API_KEY_ENCRYPTION_KEY_V${version}`, true);
  if (!/^[0-9a-f]{64}$/i.test(raw))
    throw new Error('API encryption key must be 32 bytes encoded as 64 hex characters');
  return Buffer.from(raw, 'hex');
}
export function encrypt(value: string, scope: string) {
  const nonce = randomBytes(12);
  const cipher = createCipheriv('aes-256-gcm', key(config.keyVersion), nonce);
