...
export const config = {
  ...
  cookie:
    ...
    (process.env.NODE_ENV === 'production' ? '__Host-dreamfly_session' : 'dreamfly_session_dev'),
  idle: num('SESSION_IDLE_SECONDS', 86400),
  absolute: num('SESSION_ABSOLUTE_SECONDS', 604800),
  codeTTL: num('EMAIL_CODE_TTL_SECONDS', 600),
  resend: num('EMAIL_CODE_RESEND_SECONDS', 60),
  attempts: num('EMAIL_CODE_MAX_ATTEMPTS', 5),
  ...
  uploadJSON: num('UPLOAD_JSON_MAX_BYTES', 10485760),
  uploadZIP: num('UPLOAD_ZIP_MAX_BYTES', 52428800),
  extracted: num('UPLOAD_EXTRACTED_MAX_BYTES', 104857600),
  uploadFiles: num('UPLOAD_MAX_FILES', 100),
  lease: num('WORKER_JOB_LEASE_SECONDS', 120),
  githubEnabled: process.env.GITHUB_SYNC_ENABLED === 'true',
};
if (
  config.production &&
  (!config.origin.startsWith('https://') || !config.cookie.startsWith('__Host-'))
) {
