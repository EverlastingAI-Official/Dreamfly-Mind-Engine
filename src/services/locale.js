...
export function skillShareUrl(id) {
  ...
  url.search = ''
  url.hash = `/pages/platform/index?skill=${encodeURIComponent(id)}`
  return url.href
}
...
export async function copyText(value) {
  ...
  input.remove()
  if (!copied) throw new Error(tr('无法自动复制，请手动选择 ID 复制。', 'Copy failed. Please select and copy the ID manually.'))
}
